...
export function DataTableBulkActions<TData>({
    ...
    useEffect(() => {
        if (selectedCount > 0) {
            const message = `Đã chọn ${selectedCount} ${entityName}. Thanh tác vụ hàng loạt đã sẵn sàng.`;
            ...
            >
                ...
                >
                    ...
                    <div className="flex items-center gap-x-1 text-sm" id="bulk-actions-description">
                        <Badge variant="default" className="min-w-8 rounded-lg" aria-label={`${selectedCount} đã chọn`}>
                            {selectedCount}
                        </Badge>{" "}
                        <span className="hidden sm:inline">{entityName}</span> đã chọn
                    </div>
